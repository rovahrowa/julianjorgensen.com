...
let processItem = function(passedData) {
  return new Promise(function(resolve, reject) {
    ...
    if (preparedMailContent) {
      ...
      // sendEmail.send(`The Quickbooks customer, ${passedData.customer.companyName}, we tried to send invoice ${passedData.item.Number} to, does not have an email associated or is not active...`).then(() => {
      //   resolve();
